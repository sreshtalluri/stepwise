...
**G1. ✅ DONE (2026-09-18). Gated weights downloaded** — 2.81 GB in Modal Volume `stepwise-weights`: `model.ckpt` (2109 MB) plus a bundled `assets/mhr_model.pt` (696 MB). GPU verified: L40S, CUDA 12.4, 47.2 GB VRAM. **Note:** the bundled MHR file is the parametric model, not the LOD meshes (`lod?.fbx`) needed for glTF export — confirm whether the separate ~190 MB assets.zip is still required, and read its in-zip LICENSE. Original instruction was: `facebook/sam-3d-body-dinov3` on HuggingFace is gated: login plus agreeing to share contact info under Meta's Privacy Policy. Approval is not instant and is a hard blocker. Do it first, today.
