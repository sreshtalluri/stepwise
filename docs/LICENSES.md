...
| Beat This! (`CPJKU/beat_this`, PyPI `beat-this==1.1.0`) — code **and** the `final0` weights, used by `packages/beat-detect` for beats | MIT | `MIT` | Preserve copyright/license notice if redistributed. The weights are MIT too: the upstream README says "The code and the published model weights are released under the MIT license." It also notes some training audio is copyrighted; that is the usual caveat for a trained model, and ours to judge. The weights are baked into the Modal `beat_image`, not redistributed from this repo. |
| librosa (beat-detect's kick rule) | ISC | `ISC` | Preserve copyright/license notice if redistributed. |
| PyTorch / torchaudio (CPU, beat-detect only) | BSD-3-Clause | `BSD-3-Clause` | Preserve copyright/license notice if redistributed. |
