...
| 19 | **Rate-limit and removal hashes** (`ratelimit._ip_hash`) | Pseudonymous; **fixed 2026-09-25**: now keyed by the deleted-daily analytics salt. The removal row links (requester IP hash, clip, relationship) | Not written as raw IP | Fixed 2026-09-25; `/privacy` now matches | None beyond keeping the removal row's free text short-lived (item 34) | B |
