...
2. **The rate-limit hash does not rotate daily, although `/privacy` says it does.** `/privacy`
   says the IP hash for limits uses a key that "changes every day, so one day's counts cannot be
   matched to the next". `ratelimit._ip_hash` is `HMAC(STEPWISE_IP_SALT, date|ip)`. The secret is
   fixed, and **it falls back to `DATABASE_URL` when `STEPWISE_IP_SALT` is unset**. Whoever holds
   the secret can recompute any past day's hash for a candidate IP, or brute-force the IPv4 space
   for one day. The analytics `day_hash` really is unlinkable, because its salt is deleted daily.
   The two are described as if they were the same.
3. **The public repo lists live lesson links.** `evaluation/labels/count_one.json` is in a
   **public** GitHub repo (`sreshtalluri/stepwise`). It lists six real lesson `job_id`s
   (`bhangra`, `choreo-kinjaz`, `hoodie-followcam`, `mirror`, …). A `job_id` is the lesson URL
   (`/lesson/{job_id}`) and the key for "anyone with the link can remove it". Anyone reading the
   repo can open those lessons, including the hosted video and its music, or delete them. The
   labels also outlive the removal or expiry of the lesson, because git history keeps them.
...
| 19 | **Rate-limit and removal hashes** (`ratelimit._ip_hash`) | Pseudonymous, and **re-identifiable by us** with the fixed secret. The removal row links (requester IP hash, clip, relationship) | Not written as raw IP | **`/privacy` says the key changes daily; it does not.** It falls back to `DATABASE_URL` as the secret | Either fix the copy ("a keyed hash of your IP address and the date; we keep the key secret"), or, better, use a daily random salt as analytics does. Confirm `STEPWISE_IP_SALT` is set in production | B |
...
4. **Production settings I cannot see from the repo:**
   - whether `STEPWISE_IP_SALT` is set;
