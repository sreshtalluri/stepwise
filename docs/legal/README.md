...
**A note on item 3.** `robots.txt` stops compliant crawlers fetching `/lesson/` and `/job/`, so
they never see the `noindex`. A lesson URL linked from elsewhere can still show up in results as a
bare URL. If that happens, drop those two lines from `robots.ts` and let the `noindex` do the work.
