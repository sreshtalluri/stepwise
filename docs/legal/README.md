...
| 26 | `/privacy` says the rate-limit IP hash key "changes every day"; `ratelimit._ip_hash` uses a fixed secret plus the date (and falls back to `DATABASE_URL`). Fix the copy or rotate a daily salt | SR row 19 | beta | todo | `services/motion-api/ratelimit.py` `_ip_hash`; `apps/web/lib/copy.ts` `privacy` |
| 27 | `/privacy`: say that usage notes and removal rows about a removed lesson stay until they expire (Neon 13 months, PostHog its own period); mention backend performance traces (10%) or turn them off; name Neon | SR rows 18, 20, 24 | beta | todo | `apps/web/lib/copy.ts` `privacy`; `observability.py` `traces_sample_rate` |
...
| 29 | Remove live lesson `job_id`s from the public repo's eval labels (use opaque ids) | SR row 22 | beta | todo | `evaluation/labels/count_one.json` |
...
| 31 | Processor register: DPA, region and transfer mechanism for Modal, Cloudflare, Neon, PostHog and Sentry; confirm PostHog "Discard client IP data", the Sentry region and IP storage, and `STEPWISE_IP_SALT` | SR rows 18, 20, 24 | beta | todo | |
