...
| 6 | Stop persisting `expr_params` and `pred_vertices`; stop serving `shape_params.vector` | R&P §6.2; SR row 7 | beta | done (2026-09-25: the vector is out of the JSON, and since item 28 it is no longer baked into the GLB, written to the export manifest or kept in the npz) | `vendor/fast-sam-3d-body/tools/process_clip.py` `UNREAD_PER_FRAME_KEYS` (now also `shape_params`); `motion-result.schema.json` (`vector` optional, not sent) |
...
| 28 | Body shape baked into every GLB: say so on `/privacy`, update R&P §2, then decide whether to keep it or coarsen it | SR row 7; R&P §6.7; [`body-shape-decision.md`](body-shape-decision.md) | beta (copy) / before public launch (decision) | done (2026-09-25: owner chose option (b). Every GLB now has the standard MHR surface on the dancer-sized skeleton; no shape vector is applied, written to the manifest or kept in the npz; `shape_params.source` is `default_assumed`; every stored lesson re-exported; `/privacy` says "sized to their estimated limb lengths (the body's surface is a standard shape, not theirs)"; R&P §2 finding 2 corrected. Still open for launch: the counsel question in `body-shape-decision.md` §6, via item 18) | `modal_app.py` `_export_clip_gltf`; `motion_result.py` `shape_params`; `process_clip.py` `UNREAD_PER_FRAME_KEYS`; `apps/web/lib/copy.ts` `privacy`; re-export: `services/motion-api/restandardize_bodies.py` |
