...
| # | Decision | Status | Note |
|---|---|---|---|
| D5 | Accounts: none, magic link, or OAuth? | LEANING | **Recommendation (not yet a decision): `docs/research/identity-and-analytics.md` §4.** Optional accounts, anonymous by default. Every upload mints a **creator token** held in the browser — that is what owns a lesson, and it is the "account holder" §512(i) needs. Signing in binds the browser's tokens to an email via **magic link**, so a library survives a new device; lessons created anonymously are claimed automatically because the browser already holds their tokens. No OAuth, no passkeys in v1 (passkeys stay cheap to add later, since the identity is the email either way). Keeps W7's shipped "No account. Works in your browser." literally true. Strongest justification: with no identity there is no subscriber to terminate, so §512(i)'s repeat-infringer policy is unimplementable and every DMCA safe harbour falls with it (D8). Second: retention is the one metric anonymity cannot measure, and it is what sponsors ask about. Affects A3 and A6. |
