...
| A5 | **Count anchoring ("set 1 here")** | OPEN | Detected beats will often be wrong or half/double tempo. The correction UI needs to be fast and obvious — tap on the beat, or drag the grid? What does "no music / manual counts" look like? **Partly narrowed 2026-09-20 (branch `lesson-structure`), NOT resolved.** The data side is now settled: `MotionResult.beat_proposal` carries the guess together with its confidence, its half/double alternates and its warnings; the learner's grid lives separately and always wins; and `isStillProposed()` (`packages/navigation/src/core.ts`) is the single place that answers "machine or human", so no label can claim the wrong one. What is still open is the part A5 actually asks — the *feel* of the correction. Today it is the four controls W6 already shipped ("Set count 1 here", tap-in, half, double) behind a `<details>`: a working placeholder, not a designed answer. Nothing is drag-to-anchor; the alternates are not offered as one-tap corrections even though the data for that now travels in the document; and "no music / manual counts" still has no distinct state — a silent clip just gets no proposal and the plain default grid, with no line saying so. Needs a real design pass. |
