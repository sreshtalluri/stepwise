...
| **Labels** (`labels/*.json`) | **In git** | Hand-labelled ground truth, including for uploaded lessons that are not in `clips.yaml` (keyed by a short clip name). **Never a job id**: a job id is the lesson's link, anyone holding it can open or remove the lesson, and this repo is public. `labels/count_one.json`: the owner's count 1 (one entry per dance: a label with `method: "copied"` was carried over from another copy of the same dance by /owner, so it is not a second opinion), scored by `packages/beat-detect/python/tests/test_count_one_labels.py`. Never the video. |
